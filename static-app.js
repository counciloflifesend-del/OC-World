...
const seed = {
  ...
  announcements: [{ id: "a_1", title: "System Update V2.4", body: "New relationship graph rendering tools are live. Try the Relationship Map inside a world.", pinned: true, createdAt: "2026-04-25" }]
};
...
function settingsPage() {
  return `<section class="page-stack"><div class="page-heading"><h1>Settings</h1><p>Adjust local workspace preferences and review how this prototype stores data.</p></div><div class="admin-grid"><section class="panel"><h2>${icon("⚙")}Workspace</h2><div class="settings-list"><div><strong>Current creator</strong><span>${esc(user().username)}</span></div><div><strong>Storage mode</strong><span>Local browser storage</span></div><div><strong>Admin area</strong><span>${isAdmin() ? "Unlocked" : "Locked behind admin login"}</span></div></div></section><section class="panel"><h2>${icon("⛲")}Fun Fountain Dew Roll</h2><p>Use the fountain button in the top bar to roll a d4 any time.</p><button class="primary-button" id="settingsRoll">${icon("⛲")}Roll d4</button></section><section class="panel danger-panel"><h2>${icon("!")}Local Data</h2><p>Resetting returns the app to the seeded demo data.</p><button class="danger-button" id="resetDemo">${icon("×")}Reset Local Demo</button></section></div></section>`;
}
...
function formData(container) { return Object.fromEntries(new FormData(container).entries()); }
function saveWorld() {
